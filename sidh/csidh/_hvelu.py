from sidh.csidh._poly_redc import *
...
class Hvelu(object):
    def __init__(self, _prime, _style, _verbose):
        global C_xEVAL
        global C_xISOG
        global global_L
        global verbose
        global prime
        global ML
        self.prime = prime = _prime
        self.verbose = verbose = _verbose

        self.ML = ML = poly_redc_init(prime, _style)
        self.global_L = global_L = ML.global_L

        self.C_xEVAL = C_xEVAL = list(
            map(cEVAL, global_L)
        ...
        cISOG_and_cEVAL() # FIXME: not for general use, right?
...
def KPs_s(P, A, i):
    ...
    if sJ == 1:
        ...
        hI = [
            [fp_sub(0, iP[0]), iP[1]] for iP in I
        ]  # we only need to negate x-coordinate of each point
        ptree_hI = product_tree(hI, sI)  # product tree of hI
