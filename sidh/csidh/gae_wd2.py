...
from sidh.common import geometric_serie, rounds
...
class Gae_wd2(object):
    ...
    def security(self, M, n):
        ...
        """
        return sum(list([log(2 * M[i] + 1, 2) for i in range(n)]))
        ...
        for i in range(len(strategy)):
            ...
            while (prev + strategy[k]) < (n - 1 - i):
                ...
                for j in range(prev, prev + strategy[k], 1):
                    T = list(
                        [
                            self.curve.xmul(T[0], E_i, self.formula.L.index(L[j])),
                            self.curve.xmul(T[1], E_i, self.formula.L.index(L[j])),
                        ]
                    )
            ...
            if (
                ...
                ramifications[-1][0][0], ramifications[-1][1][0] = cswap(
                    ramifications[-1][0][0], ramifications[-1][1][0], c_i
                )  # XT_+ <-> XT_-
                ramifications[-1][0][1], ramifications[-1][1][1] = cswap(
                ...
                if self.curve.isinfinity(ramifications[-1][0]) == False:
                    ...
                    if self.formula_name!= 'tvelu':
                        ...
                        if self.tuned:
                            self.formula.set_parameters_velu(
                                self.formula.sJ_list[pos], self.formula.sI_list[pos], pos
                            )

                        else:
                            # -------------------------------------------------------------
                            # Parameters sJ and sI correspond with the parameters b and b' from example 4.12 of https://eprint.iacr.org/2020/341
                            # These paramters are required in KPs, xISOG, and self.formula.xeval
                        ...
                        if (
                            ...
                            K = self.formula.kps(ramifications[-1][0], E_i, pos)

                        else:
                            self.formula.kps(ramifications[-1][0], E_i, pos)
                            T = self.curve.xmul(ramifications[-1][0], E_i, pos)
                    ...
                    ramifications[0][1] = self.curve.xmul(
                    ...
                    if self.formula_name== 'tvelu' or (
                        ...
                        ramifications[0][0] = self.formula.xeval(
                            ramifications[0][0], pos
                        )
                        ramifications[0][1] = self.formula.xeval(
                    ...
                    T[0], ramifications[0][0][0] = cswap(
                        T[0], ramifications[0][0][0], b_i
                    )
                    T[1], ramifications[0][0][1] = cswap(
                        T[1], ramifications[0][0][1], b_i
                    )
                    T_prime[0], ramifications[0][1][0] = cswap(
                        T_prime[0], ramifications[0][1][0], b_i
                    )
                    T_prime[1], ramifications[0][1][1] = cswap(
                    ...
                    for j in range(1, len(moves) - 1, 1):
                        ...
                        # T_- (or T_)
                        T = self.curve.xmul(ramifications[j][0], E_i, pos)
                        ...
                        T[0], ramifications[j][0][0] = cswap(
                            T[0], ramifications[j][0][0], b_i
                        )
                        T[1], ramifications[j][0][1] = cswap(
                        ...
                        T[0], ramifications[j][1][0] = cswap(
                            T[0], ramifications[j][1][0], b_i
                        )
                        T[1], ramifications[j][1][1] = cswap(
