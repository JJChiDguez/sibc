...
def Poly_redc(poly_mul):
    ...
    def reciprocal(f, flen, n):
        ...
        if flen < n:
            ...
            return reciprocal(f + [[0, 0]] * (n - flen), n, n)
        ...
        if n == 1:
            ...
            return [list(f[0]), fp.fp2_sub([0, 0], f[1])], fp.fp2_sqr(f[0])
            ...
            t0 = fp.fp2_sqr(f[1])
            t1 = fp.fp2_mul(f[0], f[2])
            t2 = fp.fp2_sub(t1, t0)
            t2 = fp.fp2_mul(t2, f[0])
            return (
                [fp.fp2_mul(g[0], a), fp.fp2_mul(g[1], a), fp.fp2_sub([0, 0], t2)],
                fp.fp2_sqr(a),
            )
            ...
            g, a = reciprocal(f[:2], 2, 2)
            t0 = fp.fp2_sqr(f[1])
            t1 = fp.fp2_mul(g[0], f[2])
            t2 = fp.fp2_mul(g[0], f[3])
            t3 = fp.fp2_mul(g[1], f[2])
            t0 = fp.fp2_sub(t1, t0)
            t1 = fp.fp2_add(t2, t3)
            t2 = fp.fp2_mul(t0, g[0])
            t3 = fp.fp2_mul(t0, g[1])
            t4 = fp.fp2_mul(t1, g[0])
            t3 = fp.fp2_add(t3, t4)
            return (
                [
                    fp.fp2_mul(g[0], a),
            ...
            t = [fp.fp2_sub(t[0], a) ] + t[1:n]                                         # (f * g - a)     mod x^n
            ...
            t = [ fp.fp2_mul(a, g[i]) for i in range(0, m, 1) ] + [ fp.fp2_sub(0, t[i]) for i in range(0, n - m, 1) ]
            """

            # """
            # Basic idea but saving multiplication because of f*g - a is multiple of x^m
            # t = poly_mul_xn1_middle(g, m, f[:n], n)                                  #  f * g          mod x^n (the last 'm >= n - m' coefficients)
            t = poly_mul_middle(
                g, m, f[:n], n
            ...
            # Finally, the reciprocal is a*g - (f*g - a)*g mod x^n
            t = [fp.fp2_mul(a, g[i]) for i in range(0, m, 1)] + [
                fp.fp2_sub([0, 0], t[i]) for i in range(0, n - m, 1)
            ]
            # """

            return t, fp.fp2_sqr(a)
    ...
    def poly_redc(h, hlen, f):
        ...
        if hlen < flen:
            ...
            return list(h) + [[0, 0]] * (flen - hlen - 1)

        elif flen == 2 and hlen == 2:

            t0 = fp.fp2_mul(h[0], f['poly'][1])  # h0 * f1
            t1 = fp.fp2_mul(h[1], f['poly'][0])  # h1 * f0
            return [fp.fp2_sub(t0, t1)]  # f1 * (h0 + h1*x) mod (f0 + f1*x)

        elif flen == 2 and hlen == 3:

            f0_squared = fp.fp2_sqr(f['poly'][0])  # f0^2
            f1_squared = fp.fp2_sqr(f['poly'][1])  # f1^2
            t = fp.fp2_sub(f['poly'][0], f['poly'][1])  # f0 - f1

            t = fp.fp2_sqr(t)  # (f0 - f1)^2
            t = fp.fp2_sub(t, f0_squared)  # (f0 - f1)^2 - f0^2
            t = fp.fp2_sub(t, f1_squared)  # (f0 - f1)^2 - f0^2 - f1^2 = -2*f0*f1

            f0_squared = fp.fp2_add(f0_squared, f0_squared)  # 2*(f0^2)
            f1_squared = fp.fp2_add(f1_squared, f1_squared)  # 2*(f1^2)

            t0 = fp.fp2_mul(f0_squared, h[2])  # [2*(f0^2)] * h2
            t1 = fp.fp2_mul(f1_squared, h[0])  # [2*(f1^2)] * h0
            t2 = fp.fp2_mul(t, h[1])  # [-2*f0*f1] * h1
            return [
                fp.fp2_add(t0, fp.fp2_add(t1, t2))
            ]  # [2 * (f1^2)] * (h0 + h1*x + h2*x^2) mod (f0 + f1*x)

        elif flen == 3 and hlen == 3:

            f2h1 = fp.fp2_mul(f['poly'][2], h[1])
            f2h0 = fp.fp2_mul(f['poly'][2], h[0])
            f1h2 = fp.fp2_mul(f['poly'][1], h[2])
            f0h2 = fp.fp2_mul(f['poly'][0], h[2])
            return [fp.fp2_sub(f2h0, f0h2), fp.fp2_sub(f2h1, f1h2)]
            """ 
        elif fdeg == hdeg:

            t0 = [ fp.fp2_mul(h[inner], f['poly'][fdeg - 1]) for inner in range(0, fdeg - 1, 1) ]
            t1 = [ fp.fp2_mul(h[fdeg - 1], f['poly'][inner]) for inner in range(0, fdeg - 1, 1) ]
            return [ fp.fp2_sub(t1[inner], t0[inner]) for inner in range(0, fdeg - 1, 1) ]
            """
        else:
            ...
            return [
                fp.fp2_sub(fp.fp2_mul(f['a'], h[i]), qf[i])
