from sidh.bsidh.poly_mul import *
...
def reciprocal(f, flen, n):
    ...
    if n == 1:
        ...
        return  [list(f[0]), fp2_sub([0,0], f[1])], fp2_sqr(f[0])
        ...
        t2 = fp2_sub(t1, t0)
        t2 = fp2_mul(t2, f[0])
        return [ fp2_mul(g[0], a), fp2_mul(g[1], a), fp2_sub([0,0], t2) ], fp2_sqr(a)
        ...
        t4 = fp2_mul(t1, g[0])
        t3 = fp2_add(t3, t4)
        return [fp2_mul(g[0], a), fp2_mul(g[1], a), fp2_sub([0,0], t2), fp2_sub([0,0], t3)], fp2_sqr(a)
        ...
        t = [fp2_sub(t[0], a) ] + t[1:n]                                         # (f * g - a)     mod x^n
        ...
        t = [ fp2_mul(a, g[i]) for i in range(0, m, 1) ] + [ fp2_sub(0, t[i]) for i in range(0, n - m, 1) ]
        """
        ...
        # Finally, the reciprocal is a*g - (f*g - a)*g mod x^n
        t = [ fp2_mul(a, g[i]) for i in range(0, m, 1) ] + [ fp2_sub([0,0], t[i]) for i in range(0, n - m, 1) ]
        #"""
...
def poly_redc(h, hlen, f):
    ...
    if hlen < flen:
        ...
        t0 = fp2_mul(h[0], f['poly'][1])     # h0 * f1
        t1 = fp2_mul(h[1], f['poly'][0])     # h1 * f0
        return [ fp2_sub(t0, t1) ]           # f1 * (h0 + h1*x) mod (f0 + f1*x)

    elif flen == 2 and hlen == 3:

        f0_squared = fp2_sqr(f['poly'][0])       # f0^2
        f1_squared = fp2_sqr(f['poly'][1])       # f1^2
        t = fp2_sub(f['poly'][0], f['poly'][1])  # f0 - f1
        
        t = fp2_sqr(t)                           # (f0 - f1)^2
        t = fp2_sub(t, f0_squared)               # (f0 - f1)^2 - f0^2
        t = fp2_sub(t, f1_squared)               # (f0 - f1)^2 - f0^2 - f1^2 = -2*f0*f1

        f0_squared = fp2_add(f0_squared, f0_squared) # 2*(f0^2)
        f1_squared = fp2_add(f1_squared, f1_squared) # 2*(f1^2)
        
        t0 = fp2_mul(f0_squared, h[2])           # [2*(f0^2)] * h2
        t1 = fp2_mul(f1_squared, h[0])           # [2*(f1^2)] * h0
        t2 = fp2_mul(t, h[1])                    # [-2*f0*f1] * h1
        return [ fp2_add(t0, fp2_add(t1,t2))  ]   # [2 * (f1^2)] * (h0 + h1*x + h2*x^2) mod (f0 + f1*x)
    
    elif flen == 3 and hlen == 3:

        f2h1 = fp2_mul(f['poly'][2], h[1])
        f2h0 = fp2_mul(f['poly'][2], h[0])
        f1h2 = fp2_mul(f['poly'][1], h[2])
        f0h2 = fp2_mul(f['poly'][0], h[2])
        return [fp2_sub(f2h0, f0h2), fp2_sub(f2h1, f1h2)]
        """ 
    elif fdeg == hdeg:

        t0 = [ fp2_mul(h[inner], f['poly'][fdeg - 1]) for inner in range(0, fdeg - 1, 1) ]
        t1 = [ fp2_mul(h[fdeg - 1], f['poly'][inner]) for inner in range(0, fdeg - 1, 1) ]
        return [ fp2_sub(t1[inner], t0[inner]) for inner in range(0, fdeg - 1, 1) ]
        """
    else:
        ...
        return [ fp2_sub(fp2_mul(f['a'], h[i]), qf[i]) for i in range(0, flen - 1, 1) ]
...
def reciprocal_tree(r, glen, ptree_f, n):
    ...
    if (n == 0):
        ...
        return {'left':None, 'right':None, 'poly':[1], 'deg': 0, 'reciprocal':[[1,0]], 'a':[1,0]}
