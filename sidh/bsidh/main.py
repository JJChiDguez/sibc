...
try:
    ...
    f = open(strategy_data + setting.algorithm + '-' + setting.prime  + '-' + setting.formulaes + verb)
    ...
    f = open(strategy_data + setting.algorithm + '-' + setting.prime  + '-' + setting.formulaes + verb,'w')
...
print("// kernel point generator cost:\t%2.3fM + %2.3fS + %2.3fa = %2.3fM;\n" % (RUNNING_TIME[0] / (10.0**6), RUNNING_TIME[1] / (10.0**6), RUNNING_TIME[2] / (10.0**6), measure(RUNNING_TIME) / (10.0**6)) )
print("// Public key corresponding to Alice")
set_zero_ops()
a_public, PB_a, QB_a, PQB_a = evaluate_strategy(True, PB, QB, PQB, A, Ra, SIDp[::-1], Sp, len(SIDp))
RUNNING_TIME = get_ops()
print("// isogeny evaluation cost:\t%2.3fM + %2.3fS + %2.3fa = %2.3fM;" % (RUNNING_TIME[0] / (10.0**6), RUNNING_TIME[1] / (10.0**6), RUNNING_TIME[2] / (10.0**6), measure(RUNNING_TIME) / (10.0**6)) )
...
set_zero_ops()
b_public, PA_b, QA_b, PQA_b = evaluate_strategy(True, PA, QA, PQA, A, Rb, SIDm[::-1], Sm, len(SIDm))
RUNNING_TIME = get_ops()
print("// isogeny evaluation cost:\t%2.3fM + %2.3fS + %2.3fa = %2.3fM;" % (RUNNING_TIME[0] / (10.0**6), RUNNING_TIME[1] / (10.0**6), RUNNING_TIME[2] / (10.0**6), measure(RUNNING_TIME) / (10.0**6)) )
...
RUNNING_TIME = get_ops()
print("// kernel point generator cost:\t%2.3fM + %2.3fS + %2.3fa = %2.3fM;" % (RUNNING_TIME[0] / (10.0**6), RUNNING_TIME[1] / (10.0**6), RUNNING_TIME[2] / (10.0**6), measure(RUNNING_TIME) / (10.0**6)) )
set_zero_ops()
ss_a, _, _, _ = evaluate_strategy(False, PB, QB, PQB, b_public, RB_a, SIDp[::-1], Sp, len(SIDp))
RUNNING_TIME = get_ops()
print("// isogeny evaluation cost:\t%2.3fM + %2.3fS + %2.3fa = %2.3fM;" % (RUNNING_TIME[0] / (10.0**6), RUNNING_TIME[1] / (10.0**6), RUNNING_TIME[2] / (10.0**6), measure(RUNNING_TIME) / (10.0**6)) )
...
print("// kernel point generator cost:\t%2.3fM + %2.3fS + %2.3fa = %2.3fM;" % (RUNNING_TIME[0] / (10.0**6), RUNNING_TIME[1] / (10.0**6), RUNNING_TIME[2] / (10.0**6), measure(RUNNING_TIME) / (10.0**6)) )
set_zero_ops()
ss_b, _, _, _ = evaluate_strategy(False, PA, QA, PQA, a_public, RA_b, SIDm[::-1], Sm, len(SIDm))
RUNNING_TIME = get_ops()
print("// isogeny evaluation cost:\t%2.3fM + %2.3fS + %2.3fa = %2.3fM;" % (RUNNING_TIME[0] / (10.0**6), RUNNING_TIME[1] / (10.0**6), RUNNING_TIME[2] / (10.0**6), measure(RUNNING_TIME) / (10.0**6)) )
