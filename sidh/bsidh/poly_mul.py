...
def karatsuba_mul(f, flen, g, glen):
    ...
    if flen == 3:
        ...
        if glen <= nf:
            ...
            return (
                c0[:nf]
                + [fp2_add(c0[nf + i], c1[i]) for i in range(0, glen - 1, 1)]
                + c1[(glen - 1) :]
            )
        ...
        f_mid = [
            fp2_add(f_low[i], f_high[i]) for i in range(0, nf, 1)
        ] + f_high[nf:]
        ...
        if mg < mf:
            ...
            g_mid = [
                fp2_add(g_low[i], g_high[i]) for i in range(0, mg, 1)
            ] + g_low[mg:]
            ...
            g_mid = [
                fp2_add(g_low[i], g_high[i]) for i in range(0, nf, 1)
            ] + g_high[nf:]
            fg_mid = poly_mul(f_mid, mf, g_mid, mg)

        fg_mid = [
            fp2_sub(fg_mid[i], fg_high[i]) for i in range(0, mf + mg - 1, 1)
        ] + fg_mid[(mf + mg - 1) :]
        fg_mid = [
            fp2_sub(fg_mid[i], fg_low[i]) for i in range(0, 2 * nf - 1, 1)
        ] + fg_mid[(2 * nf - 1) :]
...
for i in range(0, max_exp, 1):
    j_next *= 2
    negative_powers_of_two[j_next] = fp2_mul(
...
def qring_mul(f, g, e):
    ...
    if (n == 1) and (m <= 8):
        ...
        return [
            [fp2_mul(h[i], negative_powers_of_two[n2]) for i in range(0, m, 1)]
        ]
        # return [list(h)]
        ...
        for j in range(0, n2, 1):
            ...
            for i in range(0, m, 1):
                ...
                h2j.append(
                    fp2_op[1 - sgn]([0, 0], fp2_sub(H1[j][r], H2[j][r]))
                )
...
def poly_mul_modxn(n, f, flen, g, glen):
    ...
    if n == 4 and glen == 4:
        ...
        for i in range(0, nf, 1):
            for j in range(0, n - 2 * i - 1, 1):
                c[k] = fp2_mul(
                    fp2_add(f[i], f[i + j + 1]), fp2_add(g[i], g[i + j + 1])
                )
    ...
    fg_01 = [fp2_sub(fg_01[i], fg_0[i]) for i in range(0, n01, 1)] + fg_01[
        n01:
    ]
...
def quasi_poly_mul_middle(g, glen, f, flen):
    ...
    A = poly_mul_middle(
        g[glen0:],
        glen1,
        [fp2_add(f[i], f[i + glen1]) for i in range(0, 2 * glen1 - 1, 1)],
        2 * glen1 - 1,
    )

    if glen % 2 == 0:
        B = poly_mul_middle(
            [fp2_sub(g[glen1 + i], g[i]) for i in range(0, glen0, 1)],
            glen0,
            f[glen1 : (3 * glen1 - 1)],
            2 * glen1 - 1,
        )
    ...
    assert len(B) == glen1
    assert len(C) == glen0
    return [fp2_sub(A[i], B[i]) for i in range(0, glen1, 1)] + [
        fp2_add(C[i], B[i]) for i in range(0, glen0, 1)
    ]
...
def poly_mul_middle(g, glen, f, flen):
    ...
    if (glen == (flen // 2)) and (flen % 2 == 0):
        ...
        return [
            fp2_add(fg_low[i], fg_high[i]) for i in range(0, glen - 1, 1)
        ] + [fg_high[glen - 1]]
