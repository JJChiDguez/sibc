...
def Poly_mul(curve):
    ...
    for i in range(0, max_exp, 1):
        j_next *= 2
        negative_powers_of_two[j_next] = fp.fp2_mul(
    ...
    def karatsuba_mul(f, flen, g, glen):
        ...
        if flen == 3:
            ...
            if glen <= nf:
                ...
                return (
                    c0[:nf]
                    + [fp.fp2_add(c0[nf + i], c1[i]) for i in range(0, glen - 1, 1)]
                    + c1[(glen - 1) :]
                )
            ...
            f_mid = [
                fp.fp2_add(f_low[i], f_high[i]) for i in range(0, nf, 1)
            ] + f_high[nf:]
            ...
            fg_mid = [
                fp.fp2_sub(fg_mid[i], fg_high[i]) for i in range(0, mf + mg - 1, 1)
            ] + fg_mid[(mf + mg - 1) :]
            fg_mid = [
                fp.fp2_sub(fg_mid[i], fg_low[i]) for i in range(0, 2 * nf - 1, 1)
            ] + fg_mid[(2 * nf - 1) :]

            return (
                fg_low[:nf]
                + [fp.fp2_add(fg_low[nf + i], fg_mid[i]) for i in range(0, nf - 1, 1)]
                + [fg_mid[nf - 1]]
                + [fp.fp2_add(fg_mid[nf + i], fg_high[i]) for i in range(0, mf - 1)]
                + fg_high[(mf - 1) :]
            )
    ...
    def qring_mul(f, g, e):
        ...
        if (n == 1) and (m <= 8):
            ...
            return [
                [fp.fp2_mul(h[i], negative_powers_of_two[n2]) for i in range(0, m, 1)]
            ]
            # return [list(h)]
            ...
            for j in range(0, n2, 1):
                ...
                for i in range(0, m, 1):
                    ...
                    h2j.append(
                        fp.fp2_op[1 - sgn]([0, 0], fp.fp2_sub(H1[j][r], H2[j][r]))
                    )
    ...
    def poly_mul_modxn(n, f, flen, g, glen):
        ...
        if n == 4 and glen == 4:
            ...
            for i in range(0, nf, 1):
                for j in range(0, n - 2 * i - 1, 1):
                    c[k] = fp.fp2_mul(
                        fp.fp2_add(f[i], f[i + j + 1]), fp.fp2_add(g[i], g[i + j + 1])
                    )
        ...
        f01 = [fp.fp2_add(f0[i], f1[i]) for i in range(0, f1len, 1)] + f0[f1len:]
        g01 = [fp.fp2_add(g0[i], g1[i]) for i in range(0, g1len, 1)] + g0[g1len:]
        ...
        fg_01 = [fp.fp2_sub(fg_01[i], fg_0[i]) for i in range(0, n01, 1)] + fg_01[
            n01:
        ]
        fg_01 = [fp.fp2_sub(fg_01[i], fg_1[i]) for i in range(0, n1, 1)] + fg_01[n1:]
    ...
    def quasi_poly_mul_middle(g, glen, f, flen):
        ...
        A = poly_mul_middle(
            g[glen0:],
            glen1,
            [fp.fp2_add(f[i], f[i + glen1]) for i in range(0, 2 * glen1 - 1, 1)],
            2 * glen1 - 1,
        )
    ...
    def poly_mul_middle(g, glen, f, flen):
        ...
        if (glen == (flen // 2)) and (flen % 2 == 0):
            ...
            return [
                fp.fp2_add(fg_low[i], fg_high[i]) for i in range(0, glen - 1, 1)
            ] + [fg_high[glen - 1]]
    ...
    def poly_mul_selfreciprocal(g, glen, f, flen):
        ...
        if glen == flen and (glen % 2) == 0:
            ...
            for i in range(0, glen - 1, 1):
                h[glen + half - 2 - i] = fp.fp2_add(h[glen + half - 2 - i], h1[i])
