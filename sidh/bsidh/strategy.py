...
def evaluate_strategy(EVAL, S_in, T_in, ST_in, E, P, L, strategy, n):
    ...
    for i in range(len(strategy)):
        ...
        ramifications[-1][0], E_i[0] = fp2_cswap(
            ramifications[-1][0], E_i[0], global_L[pos] == 4
        )
        ramifications[-1][1], E_i[1] = fp2_cswap(
    ...
    # Isogeny construction
    ramifications[0][0], E_i[0] = fp2_cswap(
        ramifications[0][0], E_i[0], global_L[pos] == 4
    )
    ramifications[0][1], E_i[1] = fp2_cswap(
    ...
    if EVAL:
        ...
        if (
            setting.formulaes == 'tvelu'
            or (setting.formulaes == 'hvelu' and global_L[pos] <= HYBRID_BOUND)
            or (global_L[pos] == 4)
        ):
