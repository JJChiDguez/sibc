...
if (setting.formulaes != 'tvelu' and setting.formulaes != 'svelu' and setting.formulaes != 'hvelu') or (setting.algorithm != 'csidh' and setting.algorithm != 'bsidh'):
...
if setting.algorithm == 'csidh':
    ...
    p = (2**(exponent_of_two)) * reduce(lambda x,y : (x*y), L) - 1               # p := 4 * l_0 * ... * l_n - 1
    ...
    pp = reduce(lambda x,y : (x*y), [ Lp[i]**Ep[i] for i in range(0, np, 1)  ])
    pm = reduce(lambda x,y : (x*y), [ Lm[i]**Em[i] for i in range(0, nm, 1)  ])
    assert((p + 1) % pp == 0)
    assert((p - 1) % pm == 0)
