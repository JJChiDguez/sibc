...
def karatsuba_mul(f, flen, g, glen):
    ...
    if flen == 3:
        ...
        if glen == 2:
            ...
            c = list([0, 0, 0, 0]) # Thus c(x) = (f * g)(x) is a cubic polynomial
        ...
        if glen <= nf:
            ...
            return c0[:nf] + [ fp_add(c0[nf + i], c1[i]) for i in range(0, glen - 1, 1)] + c1[(glen - 1):] 

        mg = glen - nf
        g_low = g[:nf]
        g_high= g[nf:glen]

        f_mid = [ fp_add(f_low[i], f_high[i]) for i in range(0, nf, 1) ] + f_high[nf:]
        ...
        if mg < mf:
            ...
            g_mid = [ fp_add(g_low[i], g_high[i]) for i in range(0, mg, 1) ] + g_low[mg:]
            fg_mid = poly_mul(f_mid, mf, g_mid, nf)

        else:

            g_mid = [ fp_add(g_low[i], g_high[i]) for i in range(0, nf, 1) ] + g_high[nf:]
            fg_mid = poly_mul(f_mid, mf, g_mid, mg)

        fg_mid = [ fp_sub(fg_mid[i], fg_high[i]) for i in range(0, mf + mg - 1, 1) ] + fg_mid[(mf + mg - 1):]
        fg_mid = [ fp_sub(fg_mid[i], fg_low[i]) for i in range(0, 2*nf - 1, 1) ] + fg_mid[(2*nf - 1):]
...
for i in range(0, max_exp, 1):
    j_next *= 2
    negative_powers_of_two[j_next] = fp_mul(negative_powers_of_two[j], inverse_of_two)
    j = j_next
# ----------------------------------------------------------------------------------
...
def qring_mul(f, g, e):
    ...
    if (n == 1) and (m <= 8):   
        ...
        return [ [fp_mul(h[i], negative_powers_of_two[n2]) for i in range(0, m, 1)] ]
        #return [list(h)]
        ...
        F1, F2 = [], []
        G1, G2 = [], []
        fp_op = {0:fp_add, 1:fp_sub} # (-1)^q determines if fp_add or fp_sub will be needed
        for j in range(0, n2, 1):
            ...
            for i in range(0, m, 1):
                ...
                sgn = q % 2
                # ---
                Fj1.append(fp_op[sgn](f[j][i], f[n2 + j][r]))
                Gj1.append(fp_op[sgn](g[j][i], g[n2 + j][r]))
                # ---
                Fj2.append(fp_op[1 - sgn](f[j][i], f[n2 + j][r]))
                Gj2.append(fp_op[1 - sgn](g[j][i], g[n2 + j][r]))
...
def poly_mul(f, flen, g, glen):
    ...
    if hlen > 64:
        ...
        fg_qring = qring_mul([ff + [0]*(n - hlen)], [gg + [0]*(n - hlen)], 0)
    ...
    if n == 4 and glen == 4:
        ...
        for i in range(0, nf, 1):
            for j in range(0, n-2*i -1, 1):
                c[k] = fp_mul(fp_add(f[i], f[i + j + 1]), fp_add(g[i], g[i + j +1]))
        ...
        for i in range(1, nf, 1):
            for j in range(1, n - 2*i,1):
                c[n + 2*i - 1 + j] = fp_add(c[n + 2*i - 1 + j], c[(1 + i)*n - i**2 - 1 + j])
...
def quasi_poly_mul_middle(g, glen, f, flen):
    ...
    A = poly_mul_middle( g[glen0:], glen1,\
            [ fp_add(f[i], f[i + glen1]) for i in range(0, 2*glen1 - 1, 1) ], 2*glen1 - 1)

    if glen % 2 == 0:
        B = poly_mul_middle( [ fp_sub(g[glen1 + i], g[i]) for i in range(0, glen0, 1) ], glen0,\
                f[glen1:(3*glen1 - 1)], 2*glen1 - 1)
    else:
        B = poly_mul_middle( [g[glen0]] + [ fp_sub(g[glen1 + i], g[i]) for i in range(0, glen0, 1) ], glen1,\
                f[glen1:(3*glen1 - 1)], 2*glen1 - 1)

    C = poly_mul_middle( g[:glen0], glen0,\
            [ fp_add(f[glen1 + i], f[2*glen1 + i]) for i in range(0, 2*glen0 - 1, 1) ], 2*glen0 - 1)

    assert( len(A) == glen1)
    assert( len(B) == glen1)
    assert( len(C) == glen0)
    return [ fp_sub(A[i], B[i]) for i in range(0, glen1, 1) ] + [ fp_add(C[i], B[i]) for i in range(0, glen0, 1) ]
...
def poly_mul_middle(g, glen, f, flen):
    ...
    if (glen == (flen // 2)) and (flen % 2 == 0):
        ...
        return [ fp_add(fg_low[i], fg_high[i]) for i in range(0, glen - 1, 1) ] + [fg_high[glen - 1]]
...
def product_tree(f, n):
    ...
    if n == 1:
        ...
        return {'left':None, 'right':None, 'poly':f[0], 'deg': len(f[0]) - 1}
