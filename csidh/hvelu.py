...
def KPs_s(P, A, i):
    ...
    if sI == 1:
        ...
        I = [ list(P2) ]
        hI = [ list([fp_sub(0, P2[0]), P2[1]]) ]            # we only need to negate x-coordinate of each point
        ptree_hI = product_tree(hI, sI)                      # product tree of hI

        if not SCALED_REMAINDER_TREE:
            # Using remainder trees
            ptree_hI = reciprocal_tree({'rpoly':[1], 'rdeg':0, 'fpoly':[1], 'fdeg':0, 'a':1}, 2*sJ + 1, ptree_hI, sI)   # reciprocal of the root is used to compute sons' reciprocals
    ...
    if sJ == 1:
        ...
        hI = [ [fp_sub(0, iP[0]), iP[1]] for iP in I ]      # we only need to negate x-coordinate of each point
        ptree_hI = product_tree(hI, sI)                      # product tree of hI
    ...
    P4 = xDBL(P2, A)                                    # x([4]P)
    P2[0], P4[0] = fp_cswap(P2[0], P4[0], sJ % 2)        # Constant-time swap
    P2[1], P4[1] = fp_cswap(P2[1], P4[1], sJ % 2)        # x([4]P) <--- coditional swap ---> x([2]P)
...
def xISOG_s(A, i):
    ...
    for j in range(0, sJ, 1):
        ...
        SUB_SQUARED[j] = fp_sub(J[j][0], J[j][1])               # (Xj - Zj)
        SUB_SQUARED[j] = fp_sqr(SUB_SQUARED[j])                 # (Xj - Zj)^2
        
        ADD_SQUARED[j] = fp_add(J[j][0], J[j][1])               # (Xj + Zj)
        ADD_SQUARED[j] = fp_sqr(ADD_SQUARED[j])                 # (Xj + Zj)^2

        XZJ4[j] = fp_sub(SUB_SQUARED[j], ADD_SQUARED[j])        # -4*Xj*Zj
    ...
    for j in range(0, sJ, 1):
        ...
        tadd = fp_mul(ADD_SQUARED[j], A[1])
        tsub = fp_mul(SUB_SQUARED[j], A[1])

        # We require the double of tadd and tsub
        tadd2= fp_add(tadd, tadd)
        tsub2= fp_add(tsub, tsub)

        t1 = fp_mul(XZJ4[j], AA)                    #       A *(-4*Xj*Zj)

        # Case alpha = 1
        linear = fp_sub(t1, tadd2)                  #       A *(-4*Xj*Zj)  - C * (2 * (Xj + Zj)^2)
        EJ_0[j] = [tsub, linear, tsub]

        # Case alpha = -1
        linear = fp_sub(tsub2, t1)                  #       C * (2 * (Xj - Zj)^2) - A *(-4*Xj*Zj)
    ...
    if not SCALED_REMAINDER_TREE:
        ...
        if ptree_hI != None:
            ...
            remainders_EJ_0 = scaled_remainder_tree(fg_0[::-1], sI, [1] + [0]*(sI - 1), sI, ptree_hI, sI)
    
            poly_EJ_1 = poly_redc(poly_EJ_1, 2*sJ + 1, ptree_hI)
            fg_1 = poly_mul_middle(ptree_hI['scaled'], sI, poly_EJ_1[::-1], sI)
            remainders_EJ_1 = scaled_remainder_tree(fg_1[::-1], sI, [1] + [0]*(sI - 1), sI, ptree_hI, sI)
        else:
            remainders_EJ_0 = []
            remainders_EJ_1 = []
    ...
    # Case alpha = 1
    hK_0 = [ [fp_sub(K[k][1], K[k][0])] for k in range(0, sK, 1 )]
    hK_0 = product(hK_0, sK)                     # product of (Zk - Xk) for each k in K
    # Case alpha = -1
    hK_1 = [ [fp_add(K[k][1], K[k][0])] for k in range(0, sK, 1 )]
    hK_1 = product(hK_1, sK)                     # product of (Zk + Xk) for each k in K 
    
    # --------------------------------------------------------------
    # Now, we have all the ingredients for computing the image curve
    A24m = fp_sub(A[0], A[1])                    # A' - 2C

    A24 = fp_exp(A[0], global_L[i])                    # (A' + 2C)^l
    A24m= fp_exp(A24m, global_L[i])                    # (A' - 2C)^l

    t24m= fp_mul(hK_1, r1)                      # output of algorithm 2 with alpha =-1 and without the demoninator
    t24m= fp_sqr(t24m)                          # raised at 2
    t24m= fp_sqr(t24m)                          # raised at 4
    t24m= fp_sqr(t24m)                          # raised at 8

    t24 = fp_mul(hK_0, r0)                      # output of algorithm 2 with alpha = 1 and without the demoninator 
    t24 = fp_sqr(t24)                           # raised at 2
    t24 = fp_sqr(t24)                           # raised at 4
    t24 = fp_sqr(t24)                           # raised at 8

    A24 = fp_mul(A24, t24m)
    A24m= fp_mul(A24m, t24)
    ...
    t24m = fp_sub(A24, A24m)                    #   (A24 - A24m)
    t24m = fp_add(t24m, t24m)                   # 2*(A24 - A24m)
    t24m = fp_add(t24m, t24m)                   # 4*(A24 - A24m)

    t24 = fp_add(A24, A24)                      # 2 * A24
    t24 = fp_add(t24, t24)                      # 4 * A24
...
def xEVAL_s(P, A):
    ...
    XZ_add = fp_add(P[0], P[1])                 # X + Z
    XZ_sub = fp_sub(P[0], P[1])                 # X - Z
    
    AXZ2 = fp_mul(P[0], P[1])                   # X * Z
    t1 = fp_sqr(P[0])                           # X^2
    t2 = fp_sqr(P[1])                           # Z^2

    CX2Z2 = fp_add(t1, t2)                      #      X^2 + Z^2
    CX2Z2 = fp_mul(CX2Z2, A[1])                 # C * (X^2 + Z^2)

    AXZ2 = fp_add(AXZ2, AXZ2)                   #       2 * (X * Z)
    CXZ2 = fp_mul(AXZ2, A[1])                   # C  * [2 * (X * Z)]
    AXZ2 = fp_mul(AXZ2, AA)                     # A' * [2 * (X * Z)]

    for j in range(0, sJ, 1):

        XZj_add = fp_add(J[j][0], J[j][1])      # Xj + Zj
        XZj_sub = fp_sub(J[j][0], J[j][1])      # Xj - Zj

        t1 = fp_mul(XZ_sub, XZj_add)            # (X - Z) * (Xj + Zj)
        t2 = fp_mul(XZ_add, XZj_sub)            # (X + Z) * (Xj - Zj)

        # Computing the quadratic coefficient
        quadratic = fp_sub(t1, t2)              #   2 * [(X*Zj) - (Z*Xj)]
        quadratic = fp_sqr(quadratic)           # ( 2 * [(X*Zj) - (Z*Xj)] )^2
        quadratic = fp_mul(A[1], quadratic)     # C * ( 2 * [(X*Zj) - (Z*Xj)] )^2

        # Computing the constant coefficient
        constant = fp_add(t1, t2)               #   2 * [(X*Xj) - (Z*Zj)]
        constant = fp_sqr(constant)             # ( 2 * [(X*Xj) - (Z*Zj)] )^2
        constant = fp_mul(A[1], constant)       # C * ( 2 * [(X*Xj) - (Z*Zj)] )^2

        # Computing the linear coefficient
        # ----------------------------------------------------------------------------------------------------------
        # C * [ (-2*Xj*Zj)*(alpha^2 + 1) + (-2*alpha)*(Xj^2 + Zj^2)] + [A' * (-2*Xj*Zj) * (2*X*Z)] where alpha = X/Z
        t1 = fp_add(J[j][0], J[j][1])           #     (Xj + Zj)
        t1 = fp_sqr(t1)                         #     (Xj + Zj)^2
        t1 = fp_add(t1, t1)                     # 2 * (Xj + Zj)^2
        t1 = fp_add(t1, XZJ4[j])                # 2 * (Xj + Zj)^2 - (4*Xj*Zj) := 2 * (Xj^2 + Zj^2)
        t1 = fp_mul(t1, CXZ2)                   # [2 * (Xj^2 + Zj^2)] * (2 * [ C * (X * Z)])

        t2 = fp_mul(CX2Z2, XZJ4[j])             # [C * (X^2 + Z^2)] * (-4 * Xj * Zj)
        t1 = fp_sub(t2, t1)                     # [C * (X^2 + Z^2)] * (-4 * Xj * Zj) - [2 * (Xj^2 + Zj^2)] * (2 * [ C * (X * Z)])

        t2 = fp_mul(AXZ2, XZJ4[j])              # (2 * [A' * (X * Z)]) * (-4 * Xj * Zj)
        linear = fp_add(t1, t2)                 # This is our desired equation but multiplied by 2
        linear = fp_add(linear, linear)         # This is our desired equation but multiplied by 4
        # ----------------------------------------------------------------------------------------------------------
    ...
    for k in range(0, sK, 1):
        ...
        XZk_add = fp_add(K[k][0], K[k][1])      # Xk + Zk
        XZk_sub = fp_sub(K[k][0], K[k][1])      # Xk - Zk
        t1 = fp_mul(XZ_sub, XZk_add)            # (X - Z) * (Xk + Zk)
        t2 = fp_mul(XZ_add, XZk_sub)            # (X + Z) * (Xk - Zk)

        # Case alpha = X/Z
        hK_0[k] = [fp_sub(t1, t2)]              # 2 * [(X*Zk) - (Z*Xk)]

        # Case 1/alpha = Z/X
        hK_1[k] = [fp_add(t1, t2)]              # 2 * [(X*Xk) - (Z*Zk)]
    ...
    XX = fp_mul(hK_1, r1)                       # output of algorithm 2 with 1/alpha = Z/X and without the demoninator
    XX = fp_sqr(XX)
    XX = fp_mul(XX, P[0])

    ZZ = fp_mul(hK_0, r0)                       # output of algorithm 2 with alpha = X/Z and without the demoninator
    ZZ = fp_sqr(ZZ)
    ZZ = fp_mul(ZZ, P[1])
