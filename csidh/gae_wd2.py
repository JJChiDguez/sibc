        ...
        if v[pos] > 0:     # Maximum number of degree-l_{pos} isogeny constructions?
            ...
            if isinfinity(ramifications[-1][0]) == False:
                ...
                if setting.formulaes != 'tvelu':
                    ...
                    if setting.formulaes == 'hvelu' and global_L[pos] <= HYBRID_BOUND:
                ...
                if setting.formulaes == 'tvelu' or (setting.formulaes == 'hvelu' and global_L[pos] <= HYBRID_BOUND):
...
def strategy_block_cost(L, e):
    ...
    for j in range(len(tmp_r)):
        ...
        S_out.append(S_tmp)
        C_e = C_e + (go_C + bo_C + elligator_cost + 2.0*mul_fp_by_four) * tmp_r[j]
