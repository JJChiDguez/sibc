...
if setting.formulaes == 'tvelu':
    from bsidh.tvelu import *

elif setting.formulaes == 'svelu':
    from bsidh.svelu import *

else:
    from bsidh.hvelu import *
...
print("// PA is a full order point?\t", isfull_order(prime_factors(S, A, range(0, np, 1))))
print("// QA is a full order point?\t", isfull_order(prime_factors(T, A, range(0, np, 1))))
print("// QPA is a full order point?\t", isfull_order(prime_factors(ST, A, range(0, np, 1))))
...
print("// PB is a full order point?\t", isfull_order(prime_factors(S, A, range(np, np + nm, 1))))
print("// QB is a full order point?\t", isfull_order(prime_factors(T, A, range(np, np + nm, 1))))
print("// QPB is a full order point?\t", isfull_order(prime_factors(ST, A, range(np, np + nm, 1))))
...
for idx in range(0, np, 1):
    ...
    set_zero_ops()
    if setting.formulaes == 'tvelu' or (setting.formulaes == 'hvelu' and global_L[idx] <= HYBRID_BOUND) or (global_L[idx] == 4):
        T_p = xEVAL(T_p, idx)
    else:
        T_p = xEVAL(T_p, A)
