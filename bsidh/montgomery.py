...
try:
    ...
    # First np SDACS
    SDACS = list(map(sdac, Lp))                                     # Shortest Differential Addition Chains for each small odd prime l in Lp
    SDACS_LENGTH = [ len(sdacs) for sdacs in SDACS]
    # Next nm SDACS
    SDACS = SDACS + list(map(sdac, Lm))                             # Shortest Differential Addition Chains for each small odd prime l in Lm
    SDACS_LENGTH = [ len(sdacs) for sdacs in SDACS]
...
cMUL  = lambda l: numpy.array( [ 4.0 * (SDACS_LENGTH[global_L.index(l)] + 2), 2.0 * (SDACS_LENGTH[global_L.index(l)] + 2), 6.0 * (SDACS_LENGTH[global_L.index(l)] + 2) - 2.0] )
...
def jinvariant(A):
    ...
    num = fp2_mul(num, den)                         # j := { (A')^2 - 3 * [(C')^2] }^3 / {(A')^2 - 4 * [(C')^2] } * [(C')^4]
