...
def karatsuba_mul(f, flen, g, glen):
    ...
    if glen == 1:
        ...
        return [ fp2_mul(f[i], g[0]) for i in range(0, flen, 1) ]
    ...
    if flen == 2:
        ...
        c[0] = fp2_mul(f[0], g[0])      # coeff of x^0
        c[2] = fp2_mul(f[1], g[1])      # coeff of x^2
        f01 = fp2_add(f[0], f[1])
        g01 = fp2_add(g[0], g[1])
        c[1] = fp2_mul(f01, g01)
        c[1] = fp2_sub(c[1], c[0])
        c[1] = fp2_sub(c[1], c[2])      # coeff of x^1
        return c
    ...
    if flen == 3:
        ...
        if glen == 2:
            ...
            c = list([[0,0], [0,0], [0,0], [0,0]]) # Thus c(x) = (f * g)(x) is a cubic polynomial
            ...
            c[1] = fp2_sub(c[1], c[0])
            c[1] = fp2_sub(c[1], c[2])          # coeff of x^1
            c[3] = fp2_mul(f[2], g[1])          # coeff of x^3
            f2g0 = fp2_mul(f[2], g[0])
            c[2] = fp2_add(c[2], f2g0)          # coeff of x^2
            return c 
        ...
        if glen == 3:
            ...
            karatsuba_0 = karatsuba_mul(f[:1], 1, g[:1], 1)    # field multiplication
            karatsuba_1 = karatsuba_mul(f[1:flen], 2, g[1:glen], 2)    # multiplication of two linear polynomials

            # Middle part
            f_01 = fp2_add(f[0], f[1])
            f_02 = fp2_add(f[0], f[2])
            g_01 = fp2_add(g[0], g[1])
            g_02 = fp2_add(g[0], g[2])

            t_01 = fp2_mul(f_01, g_01)
            t_02 = fp2_mul(f_02, g_02)

            l_coeff = fp2_sub(t_01, karatsuba_0[0])
            l_coeff = fp2_sub(l_coeff, karatsuba_1[0])
            q_coeff = fp2_sub(t_02, karatsuba_0[0])
            q_coeff = fp2_sub(q_coeff, karatsuba_1[2])
            q_coeff = fp2_add(q_coeff, karatsuba_1[0])

            return list(karatsuba_0 +\
                    [l_coeff, q_coeff] +\
                    karatsuba_1[1:])
        ...
        if glen <= nf:
            ...
            return c0[:nf] + [ fp2_add(c0[nf + i], c1[i]) for i in range(0, glen - 1, 1)] + c1[(glen - 1):]

        mg = glen - nf
        g_low = g[:nf]
        g_high= g[nf:glen]

        f_mid = [ fp2_add(f_low[i], f_high[i]) for i in range(0, nf, 1) ] + f_high[nf:]
        ...
        if mg < mf:
            ...
            g_mid = [ fp2_add(g_low[i], g_high[i]) for i in range(0, mg, 1) ] + g_low[mg:]
            fg_mid = poly_mul(f_mid, mf, g_mid, nf)

        else:

            g_mid = [ fp2_add(g_low[i], g_high[i]) for i in range(0, nf, 1) ] + g_high[nf:]
            fg_mid = poly_mul(f_mid, mf, g_mid, mg)

        fg_mid = [ fp2_sub(fg_mid[i], fg_high[i]) for i in range(0, mf + mg - 1, 1) ] + fg_mid[(mf + mg - 1):]
        fg_mid = [ fp2_sub(fg_mid[i], fg_low[i]) for i in range(0, 2*nf - 1, 1) ] + fg_mid[(2*nf - 1):]

        return fg_low[:nf] +\
                [ fp2_add(fg_low[nf + i], fg_mid[i]) for i in range(0, nf - 1, 1) ] +\
                [ fg_mid[nf - 1] ] +\
                [ fp2_add(fg_mid[nf + i], fg_high[i]) for i in range(0, mf - 1) ] +\
                fg_high[(mf - 1):] 

# ----------------------------------------------------------------------------------
# Table of 2 raised to a negative power
inverse_of_two = fp2_inv([2,0]) 
...
for i in range(0, max_exp, 1):
    j_next *= 2
    negative_powers_of_two[j_next] = fp2_mul(negative_powers_of_two[j], inverse_of_two)
    j = j_next
# ----------------------------------------------------------------------------------
...
def qring_mul(f, g, e):
    ...
    if (n == 1) and (m <= 8):   
        ...
        return [ [ fp2_sub(h_0[i], h_1[i]) for i in range(0, m, 1) ] ]
        ...
        #in2 =  fp2_inv(n2)   # This inverse can be omited (we only care about polynomials with the same roots)
        ...
        for j in range(0, n2, 1):
            ...
            for i in range(0, m2, 1):
                ...
                q, r = divmod(deg, m)
                # (-1)^q determines if fp2_add or fp2_sub will be needed
                hr = H[j][i]
                h[r] = fp2_op[q % 2](h[r], hr)
                deg += 1

            degy += hm2
        
        #return [ [fp2_mul(h[i], in2) for i in range(0, m, 1)] ]
        return [ [fp2_mul(h[i], negative_powers_of_two[n2]) for i in range(0, m, 1)] ]
        #return [list(h)]
        ...
        F1, F2 = [], []
        G1, G2 = [], []
        fp2_op = {0:fp2_add, 1:fp2_sub} # (-1)^q determines if fp2_add or fp2_sub will be needed
        for j in range(0, n2, 1):
            ...
            for i in range(0, m, 1):
                ...
                sgn = q % 2
                # ---
                Fj1.append(fp2_op[sgn](f[j][i], f[n2 + j][r]))
                Gj1.append(fp2_op[sgn](g[j][i], g[n2 + j][r]))
                # ---
                Fj2.append(fp2_op[1 - sgn](f[j][i], f[n2 + j][r]))
                Gj2.append(fp2_op[1 - sgn](g[j][i], g[n2 + j][r]))
        ...
        for j in range(0, n2, 1):
            ...
            for i in range(0, m, 1):
                ...
                q, r = divmod( i - m + e2, m)
                sgn = q % 2
                h1j.append(fp2_add(H1[j][i], H2[j][i]))
                h2j.append(fp2_op[1 - sgn]([0,0], fp2_sub(H1[j][r], H2[j][r])))
...
def poly_mul(f, flen, g, glen):
    ...
    if hlen > 512:
        ...
        fg_qring = qring_mul([ff + [[0,0]]*(n - hlen)], [gg + [[0,0]]*(n - hlen)], 0)

        #return [ fp2_mul(fg_qring[0][i], in2) for i in range(0, flen + glen, 1) ]
...
def poly_mul_modxn(n, f, flen, g, glen):
    ...
    if n >= (flen + glen - 1):
        ...
        return poly_mul(f, flen, g, glen) + [ [0,0] ]*(n - flen - glen + 1)

    # At this step we ensure n <  (flen + glen - 1)
    if glen == 1:

        return [ fp2_mul(f[i], g[0]) for i in range(0, n, 1) ]
    ...
    if n == 2:
        ...
        f0g0 = fp2_mul(f[0], g[0])
        f0g1 = fp2_mul(f[0], g[1])
        f1g0 = fp2_mul(f[1], g[0])
        return [ f0g0, fp2_add(f0g1, f1g0) ]
    ...
    if n == 3:
        ...
        if glen == 2:
            ...
            f0g0 = fp2_mul(f[0], g[0])
            f1g1 = fp2_mul(f[1], g[1])
        
            f01 = fp2_add(f[0], f[1])
            g01 = fp2_add(g[0], g[1])
            t01 = fp2_mul(f01, g01)
            t01 = fp2_sub(t01, f0g0)
            t01 = fp2_sub(t01, f1g1)
        
            f2g0 = fp2_mul(f[2], g[0])
            return [ f0g0, t01, fp2_add(f1g1, f2g0) ]

        if glen == 3:
            c00  = fp2_mul(f[0], g[0])      # coeff of x^0
    ...
    if n == 4 and glen == 4:
        ...
        for i in range(0, nf, 1):
            for j in range(0, n-2*i -1, 1):
                c[k] = fp2_mul(fp2_add(f[i], f[i + j + 1]), fp2_add(g[i], g[i + j +1]))
                c[k] = fp2_sub(c[k], fp2_add(c[i], c[i + j + 1]))
                k = k + 1;

        c[n-1] = list(c[0])
        for i in range(1, nf, 1):
            for j in range(1, n - 2*i,1):
                c[n + 2*i - 1 + j] = fp2_add(c[n + 2*i - 1 + j], c[(1 + i)*n - i**2 - 1 + j])

        for i in range(1, nc, 1):
            c[n + 2*i - 1] =  fp2_add(c[n + 2*i - 1], c[i])
    ...
    f01 = [ fp2_add(f0[i], f1[i]) for i in range(0, f1len, 1) ] + f0[f1len:]
    g01 = [ fp2_add(g0[i], g1[i]) for i in range(0, g1len, 1) ] + g0[g1len:]
    ...
    fg_01 = [ fp2_sub(fg_01[i], fg_0[i]) for i in range(0, n01, 1) ] + fg_01[n01:]
    fg_01 = [ fp2_sub(fg_01[i], fg_1[i]) for i in range(0, n1, 1) ] + fg_01[n1:]
    ...
    for i in range(0, n1 - 1, 1):
        fg[2*i + 2] = fp2_add(fg[2*i + 2], fg_1[i]);

    if (2*n1 < n):
        fg[2*n1] = fp2_add(fg[2*n1], fg_1[n1 - 1]);
...
def quasi_poly_mul_middle(g, glen, f, flen):
    ...
    if glen == 1:
        return [ fp2_mul(g[0], f[0]) ]

    glen0 = glen // 2       # floor(glen / 2)
    glen1 = glen - glen0    # ceil(glen / 2)

    A = poly_mul_middle( g[glen0:], glen1,\
            [ fp2_add(f[i], f[i + glen1]) for i in range(0, 2*glen1 - 1, 1) ], 2*glen1 - 1)

    if glen % 2 == 0:
        B = poly_mul_middle( [ fp2_sub(g[glen1 + i], g[i]) for i in range(0, glen0, 1) ], glen0,\
                f[glen1:(3*glen1 - 1)], 2*glen1 - 1)
    else:
        B = poly_mul_middle( [g[glen0]] + [ fp2_sub(g[glen1 + i], g[i]) for i in range(0, glen0, 1) ], glen1,\
                f[glen1:(3*glen1 - 1)], 2*glen1 - 1)

    C = poly_mul_middle( g[:glen0], glen0,\
            [ fp2_add(f[glen1 + i], f[2*glen1 + i]) for i in range(0, 2*glen0 - 1, 1) ], 2*glen0 - 1)

    assert( len(A) == glen1)
    assert( len(B) == glen1)
    assert( len(C) == glen0)
    return [ fp2_sub(A[i], B[i]) for i in range(0, glen1, 1) ] + [ fp2_add(C[i], B[i]) for i in range(0, glen0, 1) ]
...
def poly_mul_middle(g, glen, f, flen):
    ...
    if (glen == (flen // 2)) and (flen % 2 == 0):
        ...
        return [ fp2_add(fg_low[i], fg_high[i]) for i in range(0, glen - 1, 1) ] + [fg_high[glen - 1]]
...
def poly_mul_selfreciprocal(g, glen, f, flen):
    ...
    if glen == 5 and flen == 5:
        ...
        h[0] = fp2_mul(g[0], f[0])
        g1f1 = fp2_mul(g[1], f[1])
        g2f2 = fp2_mul(g[2], f[2])
        ...
        h[4] = fp2_add(h[4], t)
        h[2] = fp2_add(h[2], t)
        h[2] = fp2_add(h[2], g2f2)
        h[3] = fp2_add(h[3], g1f1)
        h[3] = fp2_add(h[3], g2f2)
    ...
    if glen == flen and (glen % 2) == 1:
        ...
        # ---
        for i in range(0, len1, 1):
            g0[i] = fp2_add(g0[i], g1[i])
            f0[i] = fp2_add(f0[i], f1[i])
        
        for i in range(0, len1, 1):
            g0[i + 1] = fp2_add(g0[i + 1], g1[i])
            f0[i + 1] = fp2_add(f0[i + 1], f1[i])

        h01 = poly_mul_selfreciprocal(g0, len0, f0, len0)

        # Mixing the computations
        for i in range(0, 2*len0 - 1, 1):
            h01[i] = fp2_sub(h01[i], h0[i])

        for i in range(0, 2*len1 - 1, 1):
            h01[i] = fp2_sub(h01[i], h1[i])

        for i in range(0, 2*len1 - 1, 1):
            h01[i + 1] = fp2_sub(h01[i + 1], h1[i])
        
        for i in range(0, 2*len1 - 1, 1):
            h01[i + 1] = fp2_sub(h01[i + 1], h1[i])

        for i in range(0, 2*len1 - 1, 1):
            h01[i + 2] = fp2_sub(h01[i + 2], h1[i])

        for i in range(1, 2*len0 - 1, 1):
            h01[i] = fp2_sub(h01[i], h01[i - 1])
    ...
    if glen == flen and (glen % 2) == 0:
        ...
        for i in range(0, glen - 1, 1):
            h[i] = fp2_add(h[i], h0[i])

        for i in range(0, glen - 1, 1):
            h[2*glen - 2 - i] = fp2_add(h[2*glen - 2 - i], h0[i])

        for i in range(0, glen - 1, 1):
            h[half + i] = fp2_add(h[half + i], h1[i])
        
        for i in range(0, glen - 1, 1):
            h[glen + half - 2 - i] = fp2_add(h[glen + half - 2 - i], h1[i])
...
def product_tree(f, n):
    ...
    if n == 1:
        ...
        return {'left':None, 'right':None, 'poly':list(f[0]), 'deg': len(f[0]) - 1}
        ...
        return {'left':left,\
                'right':right,\
                'poly':poly_mul(left['poly'], left['deg'] + 1, right['poly'], right['deg'] + 1),\
...
def product_selfreciprocal_tree(f, n):
    ...
    if n == 1:
        ...
        return {'left':left,\
                'right':right,\
                'poly':poly_mul_selfreciprocal(left['poly'], left['deg'] + 1, right['poly'], right['deg'] + 1),\
                'deg': left['deg'] + right['deg']}
...
def product(list_g_mod_f, n):
    ...
    for j in range(1, n, 1):
        ...
        out = fp2_mul(out, list_g_mod_f[j][0])
